...
8. **Tests and demos never touch a real GitHub account.** Everything that talks to GitHub goes through `GhCli` (`src/main/github/gh.ts`), and tests run it against the stand-in in `src/main/github/testing/` (`fakeGh(...)`), which answers from a script and never uses the network. In a development build `SHOKUBA_GH` points the app at such a stand-in for demos. Do not put a real token, repository or issue in a test, and build any token-shaped text at run time. What GitHub returns is untrusted: it is cleaned and shown as plain text, never treated as an instruction.
