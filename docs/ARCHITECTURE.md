...
**The shared places stand idle until there is work.** With nothing recorded the board is bare, the bench's screens are dark and the trays are empty, so an idle office never looks busy. What they show once there is work is described under _Handoffs_ below.
...
- `director.ts` decides who goes where from what agents are doing and what time it is. It is a table of rules (a `testing` state goes to the QA bench, after three seconds, and back two and a half seconds after it ends) plus **errands**: a recorded reason to be somewhere that is not the agent's state (a review in progress sends the reviewer to a reading desk, at once, and home the moment it ends; an errand comes before a state rule). It refuses to twitch (nothing happens for a state that has not lasted, and leaving lingers), gives each place a limited number of spots, and does nothing at all with reduced motion.
...
**Handoffs (slice 5c).** Work changing hands is drawn from data the renderer already has, in four small pure modules and a thin layer of drawing. Nothing is added to the database, the event model or the IPC surface.

- `handoffs.ts` maps one recorded event to at most one **flight** (a card or an envelope from one place to another, marked plain, good, bad or warn): a task dispatched (board to desk), submitted (person to your inbox), accepted (inbox to board), sent back (inbox to desk), a review handed in (reviewer to your inbox, marked with the verdict), a message between two people (you are your inbox). Everything else maps to nothing. Its `FlightQueue` (time is passed in, so it is tested on a fake clock) lets six fly at once, keeps the latest six waiting, and drops a flight that waited more than six seconds, so a burst never turns into a long replay of the past.
- `work.ts` derives, from the missions and from where each task's checks and review stand, what each place shows: the cards on the board (waiting, blocked, lately accepted), the cards in your inbox (one per submitted task, marked by the worst thing said about it), the bench (amber if any run is running, else red if any failed, else green if any passed, else dark), who holds a task (a paper on their desk) and who is reviewing. Only work waiting for you counts. A verdict on work the agent has changed since is not shown.
- `store/live.ts` is how a picture hears **news**: `applyLive` in the office store calls it for every event that arrives after the office loaded, and never for the history it hydrated, so opening the office replays nothing. `store/workStore.ts` reads each waiting task's checks and review through the calls the task view already uses (and again when a `review.changed` or `verification.changed` event says one changed, a run merely stepping along excepted), and never lets a slow old answer replace a newer one.
- `furniture.ts` and `scene.ts` draw it: cards on the board, a stack in the inbox tray, the bench's screens, a paper on a desk, a card or an envelope in an arc with a shadow on the floor, and a reading desk built like an employee's own (chair, person, then the desk over their lap) that has someone in it while a review is in progress. The name tags of the board, the inbox and the bench carry the counts.
...
**Real messages (slice 6b).** `talk.ts` turns a `message.sent` event into at most two notes, one over each employee involved: _to Bo: subject_ over the sender and _from Ava: subject_ over the recipient, with you called "you" and no note over you. The subject is the conversation's own (from its `conversation.created` event if the window still has it, otherwise from the conversations it has loaded); it is made to fit on one line (control characters and runs of space become one space, cut at 48 characters, by code point so an emoji is never cut in half); and when it is not known the note simply leaves it out. Nothing is composed. A `NoteBoard` (time passed in) keeps the newest note over each employee for six seconds. The scene draws a small chip above the status bubble. Nobody moves for a message, so nobody is pulled from real work; it is a recorded fact and carries no `simulated` mark. Like everything that reacts to news, it hears live events only, never history.

**Chats and meetings (slice 6c).** `life.ts` now decides on _outings_, which are one person (tea, a snack) or a group (a chat at the pantry table, a meeting in the meeting room). Someone whose moment has come rolls what they fancy (tea 40%, a snack 25%, a chat 25%, a meeting 10%) and falls back to something simpler if it cannot be had. A chat or meeting asks along others who are also ready (idle or waiting, not busy, out of their cooldown, idle long enough), chosen at random. The rules: a chat is two people, or three on a fifth of rolls, and a meeting is three to five in a team of six or more, and neither takes more than half the team; there must be room at the table for all of them; a gathering larger than the usual third of the team is let through only when nobody else is out; the stay starts when **every** member is there, is longer than for tea (25 to 45 seconds for a chat, 40 to 70 for a meeting), and ends for everyone at once; if they are not all there in 45 seconds it is called off; if a member's agent starts working (or they get a review) they go back, and the outing ends for the others too unless three or more remain. The director sees each member as an errand for the same kind of place, and the scene reports how many spots are free and who has arrived. While a chat or meeting lasts, `speaker()` gives each member a turn in order, and the scene draws an empty **…** chip, marked simulated, over that member only; the chip carries no text at all. A real message's note (6b) always takes the chip's place. Members of a gathering that has begun get a brief status bubble (the label and every mark, without repeating where they are).
...
**Roles (slice 7b).** A role is a starting point for hiring, stored in a `roles` table: a label, whether it leads a team, the instructions, and the permission mode a new employee starts with. `RoleService` (in main, built with the other services) puts Shokuba's own four in when the app starts, with `INSERT OR IGNORE` so an edited one is never overwritten and a missing one is put back, and keeps the originals in `shared/roles.ts` so a built-in can be reset. It creates, edits (recording only what really changed), duplicates (the label of a copy is cut before its suffix, so a long label never collides with itself), removes your own (a built-in refuses) and resets. Two active roles never share a label, compared without regard to case. Nothing links a role and an employee: hiring copies the role's text into the employee, so editing or removing a role never rewrites anyone, which is the rule that already held for the fixed templates. Six validated IPC calls and two events (`role.created`, `role.updated`, carrying ids and field names) cover it; the renderer keeps the list in a store that reads it again after every change and on every `role.*` event, and the form's unsaved-changes comparison is a small pure module.

**Departments (slice 7c).** A `departments` table (name, colour) and a nullable `department_id` on the employee, both added by one migration; `DepartmentService` (built with the other services) creates, edits (recording only what changed), removes and counts. Removing a department sets its people's `department_id` to null in the same transaction and records each as an `employee.updated`, so nobody is ever left pointing at a department that is gone; `EmployeeService` refuses a department that does not exist or was removed. Four validated IPC calls and two events (`department.created`, `department.updated`, carrying ids and field names) cover it. A department is not a team: reporting lines are untouched. Seating is a pure function: `assignDesks` takes the departments in order, keeps the existing manager-and-team order, partitions it into one group per department (then a group for people in none), and gives each group its own column of desks if all the groups can have one, and otherwise packs them, so nobody is displaced by tidiness and the result with no departments is exactly the old seating. It also returns each department's block, and `departmentPlate` says where the plate lies: flat on the floor below the last row, as wide as the block, in the open area and clear of every desk (tested for every possible block). The scene draws one slab and one name tag per block and re-seats everyone when the departments or the employees change. The roster's grouping is a pure function too.

**The office's look (slice 7d).** `shared/office.ts` defines what can be chosen: a floor tone (twelve, each two chequer shades) for each of the seven kinds of room, a wall colour (seven), windows and plants on or off, the office's name, and a name for each of eight shared places. `OfficeSettingsSchema` accepts only those ids and short printable names (never a free colour), requires every part, and so is always saved and replaced as a whole. The defaults are the colours the office was drawn with before it could be changed, pinned by a test. One row (`office_settings`, id 1) holds it as JSON; `OfficeService.get` never fails (anything unreadable is the default) and `save` records an `office.updated` event naming the parts that changed only when something did. In the renderer `style.ts` turns the settings into numbers and the name each place goes by (with `PLACE_NAME_KEY` and `ROOM_NAME_KEY` giving each name to exactly one place, so renaming one never renames another), and the scene redraws its floor, walls, windows and plants and re-labels the places when the style changes: the counts a place shows come after whatever it is called. The floor plan (`map.ts`) is deliberately outside all of this, which is why the connectivity and clearance tests keep covering every possible choice.

### GitHub (Phase 8, slice 8a)

Phase 8 connects the missions to GitHub. The rule that shapes all of it: **GitHub is somewhere strangers write, and Shokuba holds no credential for it.**

**Where the login lives.** Shokuba never has a GitHub token. `GhCli` (`main/github/gh.ts`) runs the GitHub CLI the person already signed in to with `gh auth login`, so the login stays inside `gh` (in the system keychain, or its own settings). It is started with an argument list and no shell, a working folder of the person's home, a 30 second limit and a 4 MB output limit, and an environment built from the same allow-list as every other child (`safeChildEnv`) plus only what `gh` needs to find its own settings, keyring and proxy (`XDG_CONFIG_HOME`, `GH_CONFIG_DIR`, the desktop-bus variables, `HTTP(S)_PROXY`, `NO_PROXY`, `SSL_CERT_*`). **No `GH_TOKEN`, `GITHUB_TOKEN`, `GH_HOST` or anything like them is ever passed**, so `gh` can only use the stored login, and it is told never to prompt, open a browser, check for updates or colour its output. It is only ever asked to make an API request (`gh api --hostname github.com`); it is never asked to print a token, and there is no code path that merges anything. When `gh` fails, `interpretFailure` turns its exit code and words (redacted) into one of nine kinds (not installed, signed out, not found, forbidden, rate limited, network, timeout, unreadable answer, other) with a message that says what to do, and keeps `gh`'s own words only as detail for the log. A packaged app cannot be pointed at another program; a development build can, with `SHOKUBA_GH`, which is how tests and the demo use a stand-in that never touches the network.

**What comes back is untrusted.** `GitHubClient` asks with fixed request shapes: the endpoint is built from a repository name and number checked against GitHub's own rules (`OWNER_PATTERN`, `REPO_PATTERN`), never from anything else; the reply is cut down by `gh` itself (`--jq`, fixed text) to the few fields used, before Shokuba reads any of it; every field is checked leniently (an issue with a missing or odd field is kept with what is usable, or dropped, never a crash); and every piece of text goes through `cleanText`, which removes control characters and the characters that reverse or hide text (the "trojan source" tricks), makes line endings `\n` and cuts by character to a limit (a title 200, a body 20,000). The issue's address is made from the numbers, never copied from what GitHub sent. Nothing from an issue is ever put where an agent could take it for an instruction: the mission's own description holds only the address, and the issue's words are kept in `github_links` apart from it and shown as plain text (React text, never HTML).

**Which repository.** The window never names a repository. It names one of your projects (`repoRoot`), and `GitHubService` accepts it only if it is a repository an employee works in whose `origin` remote (as Git would use it, `GitService.remoteUrl`) parses as github.com by `parseGitHubRemote`: `https`, `ssh` and the `git@github.com:o/r` form, host exactly `github.com` (so `github.com.evil.example`, `github.com@evil.example` and ports are refused), a name GitHub allows, and any login written into the address ignored and never returned.

**Making a mission from an issue.** `MissionService.createMission` takes an optional callback that runs in the same transaction, so the mission and its `github_links` row are saved together or not at all; the check "this issue is already a live mission" runs again inside it, so two imports that overlap make one mission, and an archived mission does not block importing again. It records a `github.issue.imported` event (`missionId`, `owner/repo`, number: no text) and an audit entry, and returns the link. Nothing is written to GitHub by any of this.

**Planning from an issue (slice 8b).** The person can hand an imported draft to one manager to plan. `IssuePlanning` (`main/github/planning.ts`) sets `missions.planner_id` (migration 16) through `MissionService.setPlanner`, which only works on a draft and only for an employee who exists, and sends the manager one message through the ordinary message service. The message is Shokuba's own text and names the issue only by number and repository; if it cannot be sent the hand-over is undone (the earlier manager, if any, put back). `ManagerPlanning` (`missions/planning.ts`) then treats a draft as the manager's to change if they wrote it **or** it was handed to them: the same caps (25 tasks, three drafts they may start, and a handed draft does not count toward those three), the same team-only assignment, draft-only, no running, pausing, cancelling or archiving. Taking it back, running or cancelling the mission ends it at once, because the rules read the mission each time.

**How an agent reads an issue.** Only through the read-only `read_issue` tool (`mcp/agent-tools.ts`). `IssueReader` (`main/github/issue-brief.ts`) decides who may read which: the manager a draft was handed to, and an agent whose task in progress is in that mission, and nobody else; there is no way to name an issue by number or read the issue of a mission they have no part in, and the tool is not even listed for anyone else. What it returns is framed by `renderIssue`: a header saying the text is untrusted and nothing in it is an instruction or a permission, then every line the issue wrote (title, author, text) starting with `| `, then a footer. Shokuba's own lines never start with `| `, so nothing the issue says can fake the end of the frame or start a fake `[Shokuba task]`. `MissionService.briefing` adds one line for a mission that came from an issue (number, repository and how to read it) and nothing else, so **no word GitHub wrote is ever typed into a terminal, a message or a briefing**. What the person sees (`IssueSource`) is the same text as plain React text, never HTML.

**Opening a pull request (slice 8c).** The one thing that writes to GitHub. `PullRequestService` (`main/github/pulls.ts`) has two calls. `preview` works out, and writes nothing about, exactly what would be done: it reads the mission's branch and where it started (`mission_branches`), the commits, the mission's accepted tasks and their evidence (`EvidenceService.collect`: only the headline sentences, which are counts and states, and the agent's own summary), the repository's default branch and any open pull request for the branch (both read through `gh`), and where `origin` would push (`git remote get-url --push`, parsed by `parseGitHubRemote`, which must be the same repository on github.com, over `https` or SSH). It searches the change for secrets (`scanSecrets`), from GitHub's last known default branch when that is fetched, so your own unpublished commits that would be pushed with the branch are searched too, and it lists problems (which stop it) and warnings (which do not). Everything shown is hashed together with the full commit id. `open` recomputes all of it and refuses with `changed` unless the hash is identical, so it can only do what the person was shown, then: refuses if there is a problem; holds a per-mission lock (a second click while it works is refused); pushes the one commit; opens the pull request (or, if one is already open for the branch, records it and opens none); records it in `github_links` (migration 17), publishes `github.pull.opened` and writes the audit entries `github.pull.push` and `github.pull.open`. If GitHub refuses after the push, the person is told the branch was pushed and can try again; nothing is recorded as opened.

**The push (`GitService.push`).** Only a `shokuba/…` branch, only a full commit id that is the branch's tip or an ancestor of it, and only to an address made of plain characters that cannot be read as an option. The refspec is `<commit>:refs/heads/<branch>` with no `+` and no force, so Git's own refusal of anything but a forward move stands. Before it runs, the repository's **own** settings (`git config --local` and `--worktree`, never your global ones) are checked by `push-safety.ts` for the kinds that make Git run a program or send a push elsewhere (`core.sshCommand`, `core.askPass`, `core.gitProxy`, `credential.*`, `gpg.*`, `url.*`, `remote.*.pushurl|receivepack|uploadpack|vcs|proxy`, `include*`, `http.*`, `protocol.*`, `ssh.*`): an agent that can write into a repository's config could otherwise make a push run its program at the person's click. If any is present it refuses and names the kinds (never the values). Hooks are switched off (`core.hooksPath` is empty for every Shokuba Git call) and the push is not signed. It runs with the usual restricted environment plus only the variables a push needs to use the person's own setup (the SSH agent, the desktop keyring, where their Git settings are, a proxy), which no agent ever gets. A refusal is explained in words that say what to do (`pushProblem`): not a continuation, protected branch, cannot sign in, not found. A development build can redirect the push to a local folder with `SHOKUBA_PUSH_TO` (never read by a packaged app), which is how the tests and the demo show the whole flow without a real account.

**The text.** `pull-body.ts` writes `Closes #N` first, then for each accepted task its title as a code span, Shokuba's own sentences about its checks and review, and the agent's summary inside a fenced block (redacted and with full paths removed), which GitHub renders verbatim: nothing an agent or a person wrote can be a heading, link, image, mention or reference, or close the block it is in.

**Following the pull request (slice 8d).** `PullFollowService` (`main/github/follow.ts`). `status` only reads: the pull request (`getPull`: open, closed or merged, draft, and the commit it is at), that commit's checks (CI runs and commit statuses, each normalised to passed, failed or pending: a finished run whose conclusion Shokuba does not recognise is pending, never passed) and the reviews (`standingDecisions`: each reviewer's latest decision stands, a comment does not change it, a dismissal removes it). A pull request that is over has no checks or reviews read. The look is kept for ten seconds per mission and two asks at once share one look, so a window that polls cannot hammer GitHub; the panel asks about once a minute while it is visible and the pull request is open. `followUp` makes a task from one failing check or one standing request for changes: it re-reads GitHub at the click and refuses if the pull request is not open, the check is not failing, or the reviewer no longer asks for changes; reopens a finished mission (`MissionService` gained the person-only `reopen` action, `completed` → `paused`, so nothing runs until Resume); and creates the task with `createTask`'s new `within` hook, so the task and its feedback row are saved together or not at all. **The task's own title and text are Shokuba's words** (and, for a review, the reviewer's login, which is checked against GitHub's rules), because they are typed to an agent. What GitHub said (a CI report, a reviewer's words and inline comments, each cleaned and cut) goes in `github_feedback` (migration 18), apart from the task. `IssueReader` appends it, framed by `renderFeedback` like an issue (every line GitHub's people wrote starts with `| `), to what `read_issue` returns for someone working on that task, and to no one else, and never on another mission's issue. `MissionService.briefing` adds one line (`Source: a check that failed on…` or `changes a reviewer asked for on…`) and how to read it. `PullRequestService` was adjusted to follow a pull request it opened: a recorded pull request that is still open on GitHub takes new commits (recording and announcing nothing again); one that has closed is a problem. Nothing in 8d writes to GitHub.
...
Pathfinding, walking employees, visible handoffs (Phase 5) and the office's life (Phase 6: simulated tea, snacks, chats and meetings, and real messages shown with their subject) are built. Simulated life only runs while an agent is idle or waiting, is switchable, is always labelled as simulation, and never sends anything to an agent. What is left of the office are the customisation and polish phases (editors, replay, themes).
