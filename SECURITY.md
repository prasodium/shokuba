...
- **An evidence pack is written only where you choose, and only new files.** You pick the folder in a dialog the app shows; a page can only ask for an export, never name a path. Shokuba makes a new folder, creates each file only if it does not exist (so it never overwrites and never follows a link that is in the way), names every file itself from plain characters, removes what it made if it cannot finish, and records the export in the audit log. Text in the pack is redacted like everything else, and everything an agent or person wrote is shown as inert text so a report cannot become a link, an image or HTML. The diff is kept exactly as written, so it is scanned and the pack warns you about anything that looks like a secret instead.
...
- **An evidence pack is a record, not a proof, and it is not tamper-proof.** It is made from Shokuba's database and Git repository on your computer and is not signed, so anyone who can change those (you, another program running as you, or an agent that can reach them) can change what it says. Its diff is not redacted, so a secret in the code is in the pack: read it before you share it.
