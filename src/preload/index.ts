...
const api: ShokubaApi = {
  ...
  office: {
    get: () => ipcRenderer.invoke(IPC.officeGet),
    save: (settings) => ipcRenderer.invoke(IPC.officeSave, settings),
  },
  departments: {
    list: () => ipcRenderer.invoke(IPC.departmentsList),
    create: (input) => ipcRenderer.invoke(IPC.departmentsCreate, input),
    update: (departmentId, patch) =>
      ipcRenderer.invoke(IPC.departmentsUpdate, { departmentId, patch }),
    archive: (departmentId) => ipcRenderer.invoke(IPC.departmentsArchive, { departmentId }),
  },
  roles: {
  ...
  github: {
    ...
    pullStatus: (missionId) => ipcRenderer.invoke(IPC.githubPullStatus, { missionId }),
    pullFollowUp: (input) => ipcRenderer.invoke(IPC.githubPullFollowUp, input),
  },
  messages: {
